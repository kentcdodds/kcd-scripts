...
test('resolveIOpipeScripts resolves to iopipe-scripts if not in the iopipe-scripts package', () => {
  mockPkg({ pkg: { name: 'not-iopipe-scripts' } });
  whichSyncMock.mockImplementationOnce(() => require.resolve('../'));
  expect(require('../utils').resolveIOpipeScripts()).toBe('iopipe-scripts');
});
